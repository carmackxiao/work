...
from models import WatchAccount3
...
def create_payment(request):
  ...
  payment = paypalrestsdk.Payment({
    'intent': 'sale',
    'redirect_urls': {
      # 'return_url': 'http://192.168.1.16:8000/webapp/paypal_treasurebox_process',
      # 'cancel_url': 'http://192.168.1.16:8000/webapp/cancel'

      'return_url': 'http://115.28.69.10/webapp/paypal_treasurebox_process',
    ...
    'transactions': [{
      ...
      'description': 'Pebble Treasure Box Watchface',

      "item_list": {
        "items":[
          {
            "name":"Pebble Treasure Box Watchface",
            "description":"Pebble Treasure Box Watchface",
            "quantity":"1",
            "price":"2",
            "currency":"USD",
  ...
  if payment.create():
    ...
    print(payment.error)
    context = {}
    return render(request, 'webapp/treasurebox/paypal_error.html', context)

def paypal_payment_success(request):
    context = {'sn': '123456', 'language': 'en'}
    return render(request, 'webapp/treasurebox/paypal_success.html', context)
...
def process_payment(request):
    ...
    if payment_result:
      ...
      if payment.state == 'approved':
        ...
        result_list = WatchAccount3.objects(sn=sku)
        if len(result_list) > 0:
          account_item = result_list[0]
          account_item.valid = True
          account_item.pay_time = datetime.now()
          account_item.save()
