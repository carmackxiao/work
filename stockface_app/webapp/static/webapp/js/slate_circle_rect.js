...
function set_color_from_template(watch_color) {
	var color_dict = {
  	...
  	var value = '#' + color_dict[watch_color]['KEY_T_BG_COLOR'];
	$('#bg_color').val(value);
  	var arr = $('#bg_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_HOUR_BG_COLOR'];
	$('#hour_bg_color').val(value);
  	arr = $('#hour_bg_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_HOUR_TEXT_COLOR'];
	$('#hour_text_color').val(value);
  	arr = $('#hour_text_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_MINUTE_BG_COLOR'];
	$('#minute_bg_color').val(value);
  	arr = $('#minute_bg_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_SECOND_BG_COLOR'];
	$('#second_bg_color').val(value);
  	arr = $('#second_bg_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_MINUTE_TEXT_COLOR'];
	$('#minute_text_color').val(value);
  	arr = $('#minute_text_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_CIRCLE_LINE_COLOR'];
	$('#circle_color').val(value);
  	arr = $('#circle_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_TEXT_COLOR'];
	$('#other_text_color').val(value);
  	arr = $('#other_text_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');

  	value = '#' + color_dict[watch_color]['KEY_T_BATTERY_COLOR'];
	$('#battery_color').val(value);
  	arr = $('#battery_color_label .value');
  	arr.get(0).style.cssText = 'background-color:' + value.replace(/^0x/, '#');
}
...
(function($, Sortable) {
  ...
  $.extend($.fn, {
    ...
    itemColor: function(options){
      ...
      // var layout = [
      //   ['#005555', '#FF00AA', '#5500FF', '#AA0000'],
      //   ['#00AAAA', '#0000FF', '#000000', '#AA00FF']
      // ];
      ...
      this.each(function() {
        ...
        for(var i = 0; i < layout.length; i++) {
          for(var j = 0; j < layout[i].length; j++) {
            ...
            grid += '<img class="color-select"' +
              'data-value="' + color + '" ' +
              'style="' +
                'width:' + itemWidth + '%; ' +
                'margin:10px;" ' + 
                'src="/static/webapp/images/circle_rect_images/' + color + '.png" />'; 
          }
        }

        var $injectedColor = 
          $('<img style="width:54px;height:66px;" id="color_value" class="value" src="/static/webapp/images/circle_rect_images/'+ $color.val() +'.png"/>' +
          '<div class="clear"></div>' +
        ...
        var $valueDisplay = $('#color_value');

        $color.on('click', function(ev) {
          $item.find('.color-box-wrap').toggleClass('show');
        });

        $item.find('.color-select').on('click', function(ev) {
          ev.preventDefault();

          var value = $(this).data('value');
          
          $color.val(value);
          var img_path = '/static/webapp/images/circle_rect_images/' + value + '.png';
          $valueDisplay.attr('src', img_path);
          // $valueDisplay.css('background-color', value.replace(/^0x/, '#'));
          $item.find('.color-box-wrap').removeClass('show');
        
          // set custom colors
          set_color_from_template(value);
        })
